import pygame
import sys
from PyQt5.Qt import QMessageBox, QWidget
from tkinter import *
...
def remove10():
    ...
    for i in range(23):
        if (stack2D[i][0] > 0 and
            ...
            arrTemp.append( str(stack2D[i][0]) + ","+
                            str(stack2D[i][1]) + ","+
                            str(stack2D[i][2]) + ","+
                            str(stack2D[i][3]) + ","+
                            str(stack2D[i][4]) + ","+
...
def myrender():
    ...
    font = pygame.font.Font('HACK.TTF',30);
    remain = font.render("remain : " + str(misson_cnt) , True, (28,0,0))
    screen.blit(remain,(400,100))
